...
- **状态化角色**：通过非阻塞旁路把语音、Pi 工具和 39 个原生角色状态连接起来
...
Bridge 的 `visual_state` 是现有流式响应中的无文本旁路帧。Whispera 把它投影为 `visual.state`，不会把状态文字送入 TTS，也不会让浏览器参与 ASR、Agent、播放或打断。Pi 工具身份驱动工作状态；`show_assistant_expression` 提供 16 个语义反应和 `spin`、`bounce`、`burst` 三个一次性动作，不使用转录关键词匹配。
